      ...
      <div className="flex-1 min-h-0 p-6 overflow-auto">
        ...
        {activeTab === 'configuration' && FormComponent && (
          <FormComponent 
            process={process}
            managedFiles={files}
            fileProcessMappings={{}} // You may need to pass actual file mappings if available
            onGeneratePrompt={async (processId, formData) => {
              try {
                console.log('[ProcessPanel] Calling onGeneratePrompt with:', { processId, formData });
                const response = await onGeneratePrompt(processId || process.id, formData);
                console.log('[ProcessPanel] Response received:', response);
                
                // For test-scenario-generation, the App.jsx already handles the prompt update
                // So we don't need to call onPromptUpdate again to avoid conflicts
                if ((processId || process.id) !== 'test-scenario-generation' && response?.prompt) {
