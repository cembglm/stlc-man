...
def check_data_structure():
    ...
    if sessions:✅ process_name: {sess.get('process_name', 'N/A')}")
            print(f"      unique_test_cases_count: {sess.get('unique_test_cases_count', 0)}")
            print(f"      timestamp: {sess.get('timestamp', 'N/A')}
                    elif 'optimization_results' in output:
                        opt_res = output['optimization_results']
                        if isinstance(opt_res, dict) and 'unique_test_cases' in opt_res:
                            print(f"      ✅ Has optimization_results.unique_test_cases: {len(opt_res['unique_test_cases'])} items")
